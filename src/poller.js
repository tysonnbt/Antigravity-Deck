    ...
    try {
        ...
        for (const [cascadeId, info] of convToPoll) {
            ...
            if (info.status && info.status !== lastCascadeStatusMap[cascadeId]) {
                ...
                } catch (e) { console.error(`[poll] Push notification error for ${cascadeId?.substring(0, 8)}:`, e.message); }
                    ...
                    if (msg.status || msg.cascadeStatus) {
                        ...
                        if (convId && status) {
                            ...
                            if (status !== lastCascadeStatusMap[convId]) {
                                ...
                                } catch (e) { console.error(`[SSE] Push notification error for ${convId?.substring(0, 8)}:`, e.message); }
